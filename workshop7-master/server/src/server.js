...
var mongo_express = require('mongo-express/lib/middleware'); //part 2
// Import the default Mongo Express configuration
var mongo_express_config = require('mongo-express/config.default.js'); // part 2
//part 4
...
MongoClient.connect(url, function(err, db) {
  ...
  app.use('/mongo_express', mongo_express(mongo_express_config)); //part 2
...
app.post('/feeditem/:feeditemid/comments', validate({ body: CommentSchema }), function(req, res) {
   var body = req.body.author;
   var fromUser = getUserIdFromToken(req.get('Authorization'));
   var newCommentUpdate = {
   ...
   if (fromUser === body) {
     db.collection('feedItems').updateOne({_id: new ObjectID(req.params.feeditemid)},
       {
         $push: { comments: newCommentUpdate }
       }, function (err) {
         if (err) {
           return sendDatabaseError(res, err);
         }
         // Return a resolved version of the feed item.
         getFeedItem(new ObjectID(req.params.feeditemid), function (err, item) {
           if(err) {
             res.status(500).end();
           } else {
             // 201: Created.
 ...
 app.put('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
   ...
   if (fromUser === userId) {
     var object = {["comments." + commentIdx + ".likeCounter"]: new ObjectID(userId)};
     db.collection('feedItems').updateOne({_id: feedItemId},
       { $push: object },
       function (err) {
